...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["owl.jpg",new URL("./files/olympiclogophoto.jpg",import.meta.url)]]);
...
html`<button>Replay`
  ...
  main.variable(observer("graphic")).define("graphic", ["replay","TinyQueue","Quad","width","height","DOM","d3"], function*(replay,TinyQueue,Quad,width,height,DOM,d3)
{
  replay;
  ...
  for (let i = 0; true; ++i) {
    const q = quads.pop();
    if (q === undefined || q.score < 20) break;
    const qs = q.split();
...
function colorFromHistogram(histogram) {
  ...
  main.variable(observer("imageContext")).define("imageContext", ["FileAttachment","DOM","width","height"], function(FileAttachment,DOM,width, height){return(
